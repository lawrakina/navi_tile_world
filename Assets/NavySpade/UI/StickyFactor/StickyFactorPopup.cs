    ...
    public class StickyFactorPopup : PopupWithCondition
    {
        ...
        private static void RequestToResetBlocker()
        {
            ...
            if (_realTimeTimer.IsTimeOut())
            {
                ...
                if (SaveManager.Load<int>(PREFS_LAST_EARN_COUNT_KEY) >= config.Rewards[0].Upgrades.Length) 
        ...
        private void EarnReward()
        {
            ...
            SaveManager.Save(PREFS_LAST_EARN_COUNT_KEY, _config.Rewards[0].CurrentUpgradeIndex + 1);
            _config.Rewards[0].TryBuy();
